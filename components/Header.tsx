  ...
  return (
    <div className="relative flex-none h-40 sm:h-48 overflow-hidden z-10 group">
      <div 
        ...
        style={{ 
          backgroundImage: appCoverUrl ? `url("${appCoverUrl}")` : 'url("https://images.unsplash.com/photo-1620641788421-7a1c342ea42e?q=80&w=1974&auto=format&fit=crop")', 
      ...
      {isAdmin && (
        <div className="absolute top-4 left-4 z-40">
          <button 
            ...
            className="p-2 bg-black/60 hover:bg-black/80 text-white rounded-full backdrop-blur-md border border-white/20 opacity-50 hover:opacity-100"
          >
      ...
      <div className="absolute top-4 right-4 z-30 flex items-center gap-3">
        <div className="text-right hidden sm:block">
          <p className="text-xs text-neural-accent uppercase font-bold">{isAdmin ? 'Admin' : 'Aluno'}</p>
          <p className="text-sm font-medium text-white">{user.name}</p>
        </div>
        <button onClick={onLogout} className="p-2 bg-black/40 text-white hover:text-red-400 rounded-full">
      ...
      <div className="relative z-20 h-full flex flex-col justify-end p-6 max-w-4xl mx-auto w-full">
        <h1 className="text-2xl sm:text-3xl font-bold tracking-tighter text-white drop-shadow-lg flex items-center gap-3">
          <span className="w-2 h-8 bg-neural-accent rounded-sm"></span>Protocolo Salomão
        </h1>
      </div>
    </div>
  );
